...
2. Under **Assets**, download the file for your computer:
   - Windows (installed): `Pulse Setup <version>.exe`
   - Windows (portable, no install — see below): `Pulse-Portable-<version>.exe`
   - Mac: `Pulse-<version>.dmg`
   - Linux (already portable — see below): `Pulse-<version>.AppImage`
3. Run it.
...
### Running Pulse from a USB drive

Copy `Pulse-Portable-<version>.exe` (Windows) or `Pulse-<version>.AppImage` (Linux) onto a USB drive and run it directly from there — no install step, no admin rights needed, nothing written to the host computer. A `Pulse Data` folder appears next to the executable on the drive itself the first time you run it, holding your library and presentations, so the whole thing — app and your decks — travels with the drive between computers. (On Mac, just drag the `.app` out of the `.dmg` onto the drive instead of into Applications — same effect, since a `.app` bundle is already a self-contained portable unit.)

The one thing that's different in portable mode: auto-update is turned off (see Status below), so update by redownloading the latest portable build when you want it.

## How it works

Pulse is an Electron desktop app. When an instructor starts a classroom session, the app runs a small local web server (Express + Socket.IO) on the instructor's own machine. Students on the same network scan a QR code or type a short room code to open a plain web page with no install required, and their responses stream back to the presenter in real time over that same local server. Nothing leaves the room; there is no cloud backend.

Presentations are saved as plain `.pulse.json` files (with any images embedded as data URLs), so they're fully portable. You can copy them, back them up, or hand them to another instructor like any other file.
...
- **Portable mode**: a `Pulse-Portable-<version>.exe` build (Windows) and the AppImage (Linux, inherently portable already) run straight from a USB drive with no install step, and store their entire library and presentations in a `Pulse Data` folder next to the executable itself rather than the OS's normal per-user folders (`src/main/portable.ts`) — the whole thing travels together between computers. Detected via electron-builder's own `PORTABLE_EXECUTABLE_DIR` (Windows) / `APPIMAGE` (Linux) environment variables, with a best-effort heuristic on Mac (running from outside `/Applications`, most commonly straight off a mounted drive). Auto-update is disabled in this mode — silently rewriting an executable on a USB drive that might be write-protected, slow, or shared across several computers is a bad default. A small "Portable" badge appears on the Library screen when active. Verified with an automated test simulating the drive location end-to-end, plus running the actual built portable `.exe` from a simulated external drive and confirming `Pulse Data/library.json` was created there rather than in the normal Documents/AppData locations.
