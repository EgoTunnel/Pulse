...
export function JoinPanel({ joinInfo, participantCount }: { joinInfo: JoinInfo; participantCount: number }): JSX.Element {
  return (
    <div className="fixed right-6 top-6 z-30 w-72 rounded-2xl border border-slate-700 bg-slate-900/95 p-5 text-center shadow-2xl backdrop-blur">
      ...
      {joinInfo.qrDataUrl ? (
        <img src={joinInfo.qrDataUrl} alt="QR code to join" className="mx-auto mb-3 h-40 w-40 rounded-lg bg-white p-2" />
      ) : (
      ...
      <div className="flex items-center justify-center gap-2 rounded-lg bg-slate-800 py-2 text-sm text-slate-200">
